...
def create_pie_figure(data: pd.DataFrame, group_by: str) -> Figure:
    ...
    1. Computing the sum of sales (the "Total" column) per category of the `group_by` variable.
...
def create_sales_by_city_map(data: pd.DataFrame) -> Figure:
    ...
    1. Grouping data by the "City" column and aggregating the following function to other columns:
        - "Total": sum of values
        - "Latitude": mean
        - "Longitude": mean
    2. Creating and returning a mapbox plot (px.scatter_mapbox) to represent the cities geographically per sales importance.
       Tips: Use center={"lat": 18.7, "lon": 98.9}, zoom=5, size_max=50.
...
# TODO: Create a taipy page called "Overview" thanks to the builder (tgb)
    ...
    # TODO: Finally, add two bar figures in the page:
        # 1. One bar figure representing sales over the "Time" column
        # 2. Another bar figure representing sales over the "Date" column
